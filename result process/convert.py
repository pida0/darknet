...
def creat_result_txt(raw_txt_path, target_path, threshold=0.0):  
    # raw_txt_path为yolo按类输出的TXT的路径 target_path 为转换后的TXT存放路径
    ...
    for each_file in all_files:  # 遍历所有的原始txt文件，each_file为一个文件名，例如‘car.txt’
        ...
        for each_map in map_dic:  # 遍历当前存放信息的字典
            ...
            img_name=each_map+'.png' # 看助教给的是什么类型的
            ...
            x=each_file.split('_')
            class_name = x[3][:-4]  # 获取当前原始txt的类名
            #对直接生成的文件名进行分割，得到最终的类名“j、s、d、z、l”
            for info in map_dic[each_map]:
                #x=each_file.split('_')
                #class_name = x[3][:-4]  # 获取当前原始txt的类名
                txt_write.write(img_name+' ') # 写入图片名
                txt_write.write(class_name)  # 对目标txt写入类名
                txt_write.write(' ')  # space
...
creat_result_txt('/home/sy/Code/darknet/results',
                 '/home/sy/Code/darknet/converted_txt',
                 threshold=0.1)
